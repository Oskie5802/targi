...
# Uruchamia serwer Flask + sprawdza/instaluje wszystkie zaleznosci
# =============================================================================
...
# --- 3. Sprawdz OpenSSL (do certyfikatow SSL) ---
...
# --- 4. Wirtualne srodowisko Python ---
VENV_DIR="../../venv"

if [ ! -d "$VENV_DIR" ]; then
    echo -e "${YELLOW}[INFO] Tworzenie wirtualnego srodowiska Python...${NC}"
...
echo -e "${YELLOW}[INFO] Instaluje/aktualizuje pakiety Python...${NC}"
"$PIP" install --quiet --upgrade pip
"$PIP" install --quiet flask pyopenssl

echo -e "${GREEN}[OK]${NC} Pakiety Python zainstalowane"

# --- 5. Sprawdz akceleracje sprzetowa VAAPI (opcjonalne) ---
if [ -e /dev/dri/renderD128 ]; then
    echo -e "${GREEN}[GPU]${NC} Znaleziono /dev/dri/renderD128 – VAAPI dostepne"
    # Upewnij sie ze uzytkownik ma dostep do GPU
    if ! groups | grep -q 'video\|render'; then
        echo -e "${YELLOW}[WARN] Dodaj uzytkownika do grupy 'video' lub 'render' dla VAAPI:${NC}"
        echo -e "       sudo usermod -aG video,render \$USER && newgrp video"
    fi
    # Zainstaluj libva-drm jesli potrzeba
    if command -v vainfo &> /dev/null; then
        echo -e "${GREEN}[GPU]${NC} VAAPI info: $(vainfo 2>&1 | grep 'VAProfileH264' | head -1 | xargs)"
    fi
else
    echo -e "${YELLOW}[GPU]${NC} Brak /dev/dri/renderD128 – uzywam software encoder (libx264)"
fi
...
echo -e "${GREEN}  Wyniki (Monitor 1):    https://$(hostname -I | awk '{print $1}'):5001/leaderboard1${NC}"
echo -e "${GREEN}  Live Feed (Monitor 2): https://$(hostname -I | awk '{print $1}'):5001/leaderboard2${NC}"
